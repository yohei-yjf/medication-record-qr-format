...
- **分割制御レコード(911)対応**: 1つのQRコードに収まらないデータは自動で分割し、読み取り時に連番順で自動結合します。**1枚だけ読み取った場合も「揃っていない」ことを検出**し、足りないデータ連番を返します
...
### 分割されたQRコードが揃っているか確かめる

分割されたデータは1枚だけでも仕様どおりに解析できてしまうため、`ok` だけでは
「全部読めた」ことになりません。`complete` と `missing_sequences` で確かめます。

```python
result = read_notebook_from_pngs([Path("1.png").read_bytes()])

result.ok                 # False(シンボルが足りないため)
result.complete           # False
result.total_count        # 2
result.missing_sequences  # (2,) -- あと2枚目が必要
```

1枚の画像に複数のQRコードが写っていればすべて読み取ります(薬剤情報提供書に
並べて印刷されている場合など)。個別に取り出したい場合は
`decode_all_qr_from_png` / `decode_all_qr_from_image` が使えます。
